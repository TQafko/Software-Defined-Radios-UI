...
A Software defined radio is configured to output two files from binary data into 
two of its transmit channels. The signals are meant to be generated through other software
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    std::string args, file, file2, type, ant, subdev, ref, wirefmt, channel1, channel2;
    size_t spb;
    ...
    desc.add_options()
        ...
        ("channel1", po::value<std::string>(&channel1)->default_value("0"), "which channel to use")
        ("channel2", po::value<std::string>(&channel2)->default_value("1"), "which channel to use")
    ...
    std::vector<size_t> channel2_nums;
    if (type == "double")
        cpu_format = "fc64";
    else if (type == "float")
        cpu_format = "fc32";
    ...
    channel_nums.push_back(boost::lexical_cast<size_t>(channel1));
    stream_args.channels             = channel_nums;
    uhd::tx_streamer::sptr tx_stream = usrp->get_tx_stream(stream_args);
    
    uhd::stream_args_t stream2_args(cpu_format, wirefmt);
    channel2_nums.push_back(boost::lexical_cast<size_t>(channel2));
    stream2_args.channels             = channel2_nums;
    uhd::tx_streamer::sptr tx_stream2 = usrp->get_tx_stream(stream2_args);

    // send from file
    do {
        if (type == "double"){
            send_from_file<std::complex<double>>(tx_stream, file, spb);
            send_from_file<std::complex<double>>(tx_stream2, file2, spb);
        }
        else if (type == "float"){
            send_from_file<std::complex<float>>(tx_stream, file, spb);
            send_from_file<std::complex<float>>(tx_stream2, file2, spb);
        }
        else if (type == "short"){
            send_from_file<std::complex<short>>(tx_stream, file, spb);
            send_from_file<std::complex<short>>(tx_stream2, file2, spb);
        }
        else
