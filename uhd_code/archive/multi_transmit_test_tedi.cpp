...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    size_t spb;
    double rate, freq, freq2, gain, power, wave_freq, bw, bw2, lo_offset;
    float ampl;
    ...
    desc.add_options()
        ...
        ("freq2", po::value<double>(&freq2), "RF center frequency in Hz")
        ...
        ("bw2", po::value<double>(&bw2), "analog frontend filter bandwidth in Hz")
    ...
    std::cout << std::endl;
    std::cout << "Number of channels: " << usrp->get_tx_num_channels();
              << std::endl;
    ...
    // Test for 2 channels different parameters
    std::cout << std::endl
    std::cout << "Setting first channel!"
              << std::endl
    uhd::tune_request_t tune_request(freq, lo_offset);
    usrp->set_tx_freq(tune_request, 0);
    usrp->set_tx_power_reference(power - wave_table.get_power(), 0);
    usrp->set_tx_gain(gain, 0);
    usrp->set_tx_bandwidth(bw, 0);
    usrp->set_tx_antenna(ant, 0);
    std::cout << std::endl
    std::cout << "Setting Second Channel!"
              << std::endl
    uhd::tune_request_t tune_request2(freq2, lo_offset);
    usrp->set_tx_freq(tune_request2, 1);
    usrp->set_tx_power_reference(power - wave_table.get_power(), 1);
    usrp->set_tx_gain(gain, 1);
    usrp->set_tx_bandwidth(bw2, 1);
    usrp->set_tx_antenna(ant, 1);

    std::cout << std::endl
    std::cout << "Set both channels!"
              << std::endl
